...
public abstract class AlgoritmoGenetico {
	...
	public Individuo ejecuta() {
		for (int i = 0; i < pasos; i++) {
			...
			Cromosoma c1 = poblacion.getIndividuo(a1).getCromosoma();
			Cromosoma c2 = poblacion.getIndividuo(a2).getCromosoma();
			Cromosoma resultante = recombinar(c1, c2);
			resultante.mutar(probMutacion);
			Individuo individuo = new Individuo(resultante, problema);
			poblacion.reemplaza(individuo);
