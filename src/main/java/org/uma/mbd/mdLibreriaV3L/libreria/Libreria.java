...
public class Libreria {
    ...
    private int posicionLibro(String autor, String titulo) {
        int pos = 0;
        while (pos < libros.size() && !(libros.get(pos).getAutor().equalsIgnoreCase(autor) && libros.get(pos).getTitulo().equalsIgnoreCase(titulo))){
            pos++;
        }
        return (pos < libros.size()) ? pos: -1; /*Operador ternario*/
    }
